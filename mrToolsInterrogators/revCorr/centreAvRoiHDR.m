function centreAvRoiHDR(thisView,overlayNum,scanNum,x,y,s,roi)
% glmPlot.m
%
%        $Id$
...
for iPlot = 1:length(roi)+1
  ...
  if iPlot==1 %this is the voxel data
    ...
    nOverlays = analysisParams.numberContrasts;
    resolution = 2;
    ...
    HDRS = nan(glmData.nHrfComponents,nTuningCurvesIndices,prod(overlaySize));
    ...
    figure;
    for i = 1:glmData.nHrfComponents
    ...
    figure; waterfall([1-nOverlays:1/resolution:nOverlays-1],[1:1:glmData.nHrfComponents],averageHDRTunindCurves);
    figure; surf([1-nOverlays:1/resolution:nOverlays-1],[1:1:glmData.nHrfComponents],averageHDRTunindCurves);
    a = permute(averageHDRTunindCurves, [2 1 3]);
    figure; surf([1:1:glmData.nHrfComponents],[1-nOverlays:1/resolution:nOverlays-1],a);
...
%% save ehdr of ROI
% if iPlot==1
%     saveEhdr(e,analysisParams.saveName,[x,y,s],[],[])
% else
%     saveEhdr(e,analysisParams.saveName,[],roi(roiNum),nTotalVoxels)
% end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%% function to save ehdr  %%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function saveEhdr(e,analysisName,voxelID,roi,nTotalVoxels)
if ieNotDefined('roi')
    voxelID = num2str(voxelID);
    filename = [analysisName '_voxelID_',voxelID];
    save(['eHDR_' filename],'e');
else
    filename = roi{1}.name;
    save(['eHDR_' filename],'e','roi','nTotalVoxels');
end
