function glmSaveEstimates(thisView,overlayNum,scanNum,x,y,s,roi)
% glmPlot.m
%
%        $Id$
...
for iPlot = 1:length(roi)+1
  ...
  if iPlot==1 %this is the voxel data
    ...
    % save estimated values (e)
    saveEhdr(e,[],glmData,analysisParams,[x,y,s],[],[])
...
%       end
    ...
    % save estimated values (e)
    saveEhdr(e,f,glmData,analysisParams,[],roi(roiNum),nTotalVoxels)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function saveEhdr(e,f,glmData,analysisName,voxelID,roi,nTotalVoxels)
if ieNotDefined('roi')
    voxelID = num2str(voxelID);
    filename = [analysisName.saveName analysisName.groupName '_scanNum=' num2str(glmData.scanNum) '_voxelID_',voxelID];
    save(['eGLM_' filename],'e');
else
	filename = [analysisName.saveName analysisName.groupName '_scanNum=' num2str(glmData.scanNum) roi{1}.name];
    save(['eGLM_' filename],'e','f','roi','nTotalVoxels');
end
