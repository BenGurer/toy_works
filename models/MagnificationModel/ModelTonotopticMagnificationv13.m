...
for iTonoMap = 1:length(TonotopicMagnification)
    ...
    for iStimSet = 1:length(StimulusSetSpacing)
        ...
        for z = 1:length(StimulusSet)
            NeuronalResponse(z,:) = lcfModelNeuronalResponse(StimFreq(z),StimLev(z),NeuronalProperties.CF,NeuronalProperties.p,NoiseLev);
        ...
        for z = 1:nVoxels
            VoxelResponse(z) = mean (NeuronalResponse(1 + (NeuronsPerVoxel .* (z-1)) : (NeuronsPerVoxel .* (z-1)) + NeuronsPerVoxel));
        end
