...
for iVMD = 1:length(VoxelMagSpace)
    ...
    for iSMD = 1:length(StimMagDomain)
        ...
        for i = 1:length(StimFreq)
            ...
            subplot(2,2,iSMD);
            plot(data.VoxelProp.VFreqC,SMD.VoxRes(i,:)), set(gca,'XScale','log'),title (StimSpacingNames{iSMD}), xlabel('Actual Voxel Characterisic Frequency (kHz)'), ylabel('Response Level'), axis tight
            hold on
