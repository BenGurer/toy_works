...
for iVMD = 1:length(VoxelMagSpace)
    % Spacing Properites
    data.VoxelProp = VoxelProperties(LowFreq,HighFreq,nVoxels,C,VoxelMagSpace{iVMD},VoxelMagSpace2Freq{iVMD},VoxelFilterWidth{iVMD});    %     (LowFreq,HighFreq,nVoxels,C,magDomain,magScale2F,magFilterWidth)
    ...
    for iSMD = 1:length(StimMagDomain)
        ...
        for i = 1:length(StimFreq)
            SMD.VoxRes(i,:) = VoxelResponse (StimFreq(i),StimLev,data.VoxelProp.VFreqC,data.VoxelProp.p);
            subplot(2,2,iSMD);
            plot(data.VoxelProp.VFreqC,SMD.VoxRes(i,:)), set(gca,'XScale','log'),title (StimSpacingNames{iSMD}), xlabel('fc (kHz)'), ylabel('excLev'), axis tight
            hold on
    ...
    for iVMD = 1:length(VoxelMagSpace)
        ...
        for iSMD = 1:length(StimMagDomain)
            subplot(2,2,iSMD);
            plot (1:nVoxels,data.VoxelProp.VFreqC, 'r'), axis tight
            hold on
            scatter (1:nVoxels,data.VMD{iVMD,1}{iSMD ,1}.VCharaFreq)
            set(gca,'YScale','log'), xlabel('Frequency Gradient (mm)'), ylabel('Characterisic Frequency (kHz)'),title (StimSpacingNames{iSMD}),legend('Actual','Measured','Location','SouthEast')
        end
    end
...
function FreqDis = FreqDisMagDom(f)
%% number of Freq Selectivity units
FreqDis = f;
end
function f = FreqDisMagDom2Freq(FreqDis)
f = FreqDis;
end