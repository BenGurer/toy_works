...
nStimuli = 7;
StimdB = 1;    % presentaiton level (dB SPL)
...
TWn = 0.6; % Tuning width at Fn in perceptage
CorticalDistance = 30; % length of gradient in mm
NeuronDensity = 10; % number of neurons per mm
...
C = 1;
NoiseLev = 0; % level of noise at neuron level - keep 0
% NoisePercent = 15; % noise in percent
...
TWn = erb; % Tuning width at Fn in perceptage
...
%             NoiseLev = StimdB*NoisePercent;
            ...
            noise = NoisePercent/100;
            %                NoisePercent = 15/100;
            NoiseLev = StimdB*noise;
...
%                 hold on
    ...
    set(fCF,'OuterPosition',FigSize)
        for iTonoMap = 1:length(TonotopicMagnification)
%         subplot (length(TonotopicMagnification), 1, iTonoMap)
        plot (TonotopicMap(iTonoMap).CF, 1:length(TonotopicMap(iTonoMap).CF), 'color',SpacingColour{iTonoMap}, 'LineWidth', 2)
                hold on
        set(gca,'XScale','log');
        ylabel('Cortical Distance', 'FontSize', fFontSize), xlabel('CF (kHz)', 'FontSize', fFontSize);
        xlim ([LowFreq HighFreq]),ylim ([0 length(TonotopicMap(iTonoMap).CF)])
        warning('off','MATLAB:legend:IgnoringExtraEntries')
        fLeg = legend((TonotopicNames{iTonoMap}),'Location','SouthEast');
        set(fLeg, 'FontSize', fFontSize./2);
    end
        ...
        for iStimSet = 1:length(StimulusSetNames)
            if iStimSet == 1
                ...
                uicontrol('Style', 'text',...
                    ...
                    'Position', [(0.5-(szW/2)) (0.5-(szH/2)) szW szH]);
            end
...
ESTpCF = zeros(size(Response,2),1);
