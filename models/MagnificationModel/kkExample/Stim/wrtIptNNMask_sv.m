function data = wrtIptNNMask_sv(partNam,opt)
% wrtIptNNMask_sv(partNam); partNam = e.g., 'OT'; opt = 'SIM' or 'FWD'
...
if ~isempty(dir(datFile))
    ...
    if ~any(isnan(cellfun(@(x)x,data(2:end,length(fieldnames(ePars))+1))))
        ...
        if strcmp(upper(opt),'SIM')
            jwd = [jwd;repmat(num2cell(cellfun(@(x)x+35-SNR1dB,data(2:end,length(fieldnames(ePars))+1)))',1,N1)]; % MaskL (masker should be able to mask signal at 10 dB SL; dynamic range is ~25 dB);
        else
            jwd = [jwd;repmat(num2cell(cellfun(@(x)x+35-SNR1dB-10*log10(MaskDur/(2*Gate)),data(2:end,length(fieldnames(ePars))+1)))',1,N1)]; % MaskL (masker should be able to mask signal at 10 dB SL; dynamic range is ~25 dB);
        end            
        ...
        iptNam = sprintf('Ipt%sNNMask_%s_sv',upper(opt),partNam);
