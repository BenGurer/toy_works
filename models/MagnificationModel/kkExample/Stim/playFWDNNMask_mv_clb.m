function playFWDNNMask_mv_clb(item)
...
case('Browse')
    iptFile = uigetfile('IptFWD*_mv.mat','Condition parameter file');
    if ~isnumeric(iptFile)&&~isempty(dir(iptFile))
        ePars = lcfLoadIptFile(iptFile);
        if isempty(ePars)
...
MaskL = min(SigL-10*log10(MaskDur/(2*Gate))-SNR1dB-5*log10((1+P*MaskG)*exp(-P*MaskG))-25,MaxL);
