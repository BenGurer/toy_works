function playSIMNNMask_sv_clb(item)
...
case('Browse')
    iptFile = uigetfile({'IptQuiet*_sv.mat','Quiet thresholds';'IptSIM*_sv.mat','Masked thresholds'},'Condition parameter file');
    if ~isnumeric(iptFile)&&~isempty(dir(iptFile))
        ePars = lcfLoadIptFile(iptFile);
        if isempty(ePars)
...
if ~isnan(MaskG)
    SigL = min(max(MaskL+SNR1dB+5*log10((1+P*MaskG)*exp(-P*MaskG))+25,SigL0),MaxL); % The signal det. threshold would be expected to decrease with 
%     10*log10((1+P*MaskG)*exp(-P*MaskG)); here, half of that decrease is taken into account.  
else
    SigL = SigL0; MaskL = 0;
end
