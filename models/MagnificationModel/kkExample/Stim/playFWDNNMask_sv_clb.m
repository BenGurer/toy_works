function playFWDNNMask_sv_clb(item)
...
case('Browse')
    iptFile = uigetfile({'IptQuiet*_sv.mat','Quiet thresholds';'IptFWD*_sv.mat','Masked thresholds'},'Condition parameter file');
    if ~isnumeric(iptFile)&&~isempty(dir(iptFile))
        ePars = lcfLoadIptFile(iptFile);
        if isempty(ePars)
...
case('SigL0')
    SigL0 = min(eval(get(gcbo,'String')),IPars.MaxL);
    set(gcbo,'String',sprintf('%g',SigL0))
case('MaxL')
    IPars.MaxL = min(eval(get(gcbo,'String')),100); SigL0 = min(SigL0,IPars.MaxL);
    set(gcbo,'String',sprintf('%g',IPars.MaxL)), set(H2.SigL0,'String',sprintf('%g',SigL0))
case('NRvsls')
...
case('Start')
    ...
    set(H2.SigL0,'Enable','off'), set(H2.MaxL,'Enable','off'), set(H2.NRvsls,'Enable','off'), set(H2.NPractice,'Enable','off'), set(H2.ISI,'Enable','off'), set(H2.ITI,'Enable','off')
    ...
    while CTrack<=NTracks        
        ...
        Pars = orderfields(Pars); Pars.MaskL = min(Pars.MaskL,Pars.MaxL);
        ...
        strg = get(H2.MaskL,'String'); set(H2.MaskL,'String',[strg(1:strfind(strg,':')) sprintf(' %g',Pars.MaskL)]) ;      

        strg = get(get(H1.Fig1,'Title'),'String'); set(get(H1.Fig1,'Title'),'String',[strg(1:strfind(strg,':')) sprintf(' %d',CTrack)]);    
       
        lcfTrack(CTrack,Pars,datFile,SigL0,SF,NomL,HB7Gain,Nfft,RsBx)
        figure(HRsBx)
        for I = 1:3
        ...
        elseif ItrSess
            ...
            set(H2.SigL0,'Enable','on'), set(H2.MaxL,'Enable','on'), set(H2.NRvsls,'Enable','on'), set(H2.NPractice,'Enable','on'), set(H2.ISI,'Enable','on'), set(H2.ITI,'Enable','on')
...
% ********** lcfTrack **********
function lcfTrack(CTrack,Pars,datFile,SigL0,SF,NomL,HB7Gain,Nfft,RsBx)
...
if ~isempty(dir(datFile))
    ...
    pars = ([]);
    rvsls = {};
    rslts = struct('sigL',{},'idxC',{},'idxF',{},'idxR',{});
end
...
if ~isnan(MaskG)
    SigL = min(max(MaskL+10*log10(MaskDur/(2*Gate))+SNR1dB+10*log10((1+P*MaskG)*exp(-P*MaskG))+25,SigL0),MaxL);
else
    SigL = SigL0; MaskL = 0;
end
dSigL = zeros(1,NRvsls+1); dSigL(1) = 10; dSigL(2) = 5; dSigL(3:end) = 2.5;

invoke(RP2,'SetTagVal','Amp',1);
invoke(RP2,'SetTagVal','Ear',ear(Ear).chan);    
invoke(RP2,'SetTagVal','StimSize',round(MaskDur*SF)+4*round(Gate*SF));    
...
while NC+NF<NPractice 
    CTrial = CTrial+1;
    invoke(RP2,'WriteTagV','Mask',0,lcfGate(mask,MaskDur,Gate,SF)*10^((MaskL-Lee-NomL-HB7Gain)/20));
    invoke(RP2,'WriteTagV','SigMask',0,[lcfGate(mask,MaskDur,Gate,SF)*10^((MaskL-Lee-NomL-HB7Gain)/20) lcfGate(sig,0,Gate,SF)*10^((SigL-NomL-HB7Gain)/20)]);
    ...
    if Int==1          
        ...
        while etime(clock,T0)<(MaskDur+4*Gate)/1000, end   
    ...
    T0 = clock; Rslts.sigL = [Rslts.sigL SigL];
    if Resp==Int
    ...
    if ~or(ItrSess,ItrTrack)     
        ...
        if Resp==Int
            ...
            if all(resp)
                resp(:) = 0; flag(end) = 1; 
                if abs(diff(flag))==2
                    CRvsl = CRvsl+1; Rslts.idxR = [Rslts.idxR CTrial]; Rvsls(CRvsl+1) = SigL;
                end
                SigL = SigL-dSigL(CRvsl+1);
            end
        else
        ...
        figure(HMain)
        plot(H1.Fig2,1:CTrial,Rslts.sigL,'k-','LineWidth',1.3)
        plot(H1.Fig2,Rslts.idxC,Rslts.sigL(Rslts.idxC),'kv','MarkerSize',6)
        plot(H1.Fig2,Rslts.idxF,Rslts.sigL(Rslts.idxF),'k^','MarkerSize',6)
        plot(H1.Fig2,Rslts.idxR,Rslts.sigL(Rslts.idxR),'o','MarkerSize',8,'MarkerFaceColor','none')
...
fprintf(FId,'Rslts = struct(''SigL'',[');fprintf(FId,' %g',Rslts.sigL);fprintf(FId,'],...\n');
