...
if graphselect == 1
    ...
    for iTonoMap = 1:length(TonotopicMagnification)
        ...
        set(gca,'YScale','log');
        xlabel('Cortical Distance (mm)', 'FontSize', fFontSize), ylabel('pCF (kHz)', 'FontSize', fFontSize);
        fLeg = legend('Actual',[StimulusSetNames{1},' - RMSE = ',num2str(TonotopicMap(iTonoMap).RMSEPruned(iStimSet))],(StimulusSetNames{2}),(StimulusSetNames{3}),(StimulusSetNames{4}),'Location','SouthEast');
        set(fLeg, 'FontSize', fFontSize./2);
    end
end
