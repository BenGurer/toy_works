...
for iTonoMap = 1:length(TonotopicMagnification)
    ...
    TonotopicMap(iTonoMap).Qnr = NeuronalProperties.Q ;
...
fCF = figure('Color', [1 1 1]);
for iTonoMap = 1:length(TonotopicMagnification)
    plot (TonotopicMap(iTonoMap).CF,1:nNeurons, 'color',SpacingColour{iTonoMap}, 'LineWidth', 2)
    hold on
    set(gca,'XScale','log');
...
for iTonoMap = 1:length(TonotopicMagnification)
    plot (TonotopicMap(iTonoMap).CF,TonotopicMap(iTonoMap).Qnr, 'color',SpacingColour{iTonoMap}, 'LineWidth', 2)
    hold on
...
step = 1./nNeurons;
Qnr = 1./(diff(TonotopicMagnification(properties.CF))./step);
% Qnr = [Qnr(1) Qnr(1:end)];
Qnr = sum(Qnr);
properties.Q = Q./Qnr;
% Q = Q(1:end-1)+step./2;
properties.p = C*4*properties.CF ./ properties.Q; % Filter coefficient of each Voxel % VTuningWidth - population tuning width / populationERB
