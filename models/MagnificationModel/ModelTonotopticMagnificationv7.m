...
for iTonoMap = 1:length(TonotopicMagnification)
    ...
    for iStimSet = 1:length(StimulusSetSpacing)
        for i = 1:length(StimulusSet)
            neuronalResponse(i,:) = NeuronalReponse(StimFreq(i),StimLev,data.VoxelProp(iVMD).VFreqC,data.VoxelProp(iVMD).p,NoiseLev);
...
%% Local Functions
function TonotopicMap = CreateTonotopicProperties(LowFreq,HighFreq,nNeurons,C,TonotopicMagnification,TonoMag2Freq,TuningWidth,nr);    %(LowFreq,HighFreq,nVoxels,C,magDomain,magScale2F,magFilterWidth)
    
TonotopicMap.TonotopicMagnification = linspace(TonotopicMagnification(LowFreq),TonotopicMagnification(HighFreq),nNeurons); % Linearly space in magnification scale domain (ERB, FS, LOG, LIN)
TonotopicMap.CharacterisicFrequencies = TonoMag2Freq(TonotopicMagnification);   % Voxel Characterisic Frequency - Convert maginification scale values to frequency values (kHz)
TonotopicMap.TuninnWidth = TuningWidth(CharacterisicFrequencies); % Voxel frequency tuning width for each Voxel Charactersic Frequency (for each voxel)(Bandwidth in?)
TonotopicMap.TuninnWidth = TonotopicMap.TuninnWidth ./ nr;
TonotopicMap.p = C*4*TonotopicMap.CharacterisicFrequencies./TonotopicMap.TuninnWidth; % Filter coefficient of each Voxel % VTuningWidth - population tuning width / populationERB
end
