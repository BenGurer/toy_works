...
for iVMD = 1:length(VoxelMagSpace)
    ...
    for iSMD = 1:length(StimMagDomain)
        ...
        for i = 1:length(StimFreq)
            ...
            plot(data.VoxelProp(iVMD).VFreqC,SMD.VoxRes(i,:), 'color',[(StimFreq(i)./max(StimFreq)) 0.5 0.5]), set(gca,'XScale','log'), xlabel('Actual Voxel Characterisic Frequency (kHz)'), ylabel('Response Level'), axis tight, ylim ([0 StimLev(i)]);
            hold on
            line(repmat(StimFreq(i),1,2),[min(ylim) StimLev(i)], 'color',[(StimFreq(i)./max(StimFreq)) 0.5 0.5])
        end
        text(iSMD./4, 0.2, num2str(StimSpacingNames{iSMD}))
        text(0.2, iSMD./4, num2str(StimSpacingNames{iSMD}))
...
%             set(gca,'XScale','log'), title (StimSpacingNames{iSMD}), xlabel('Actual Voxel Characterisic Frequency (kHz)'), ylabel('Response Level'), axis tight, ylim ([0 StimLev(i)]), xlim ([0 20]);          
            ...
            plot(repmat(StimFreq(i),1,2),[min(ylim) StimLev(i)], 'color',[(StimFreq(i)./max(StimFreq)) 0.5 0.5], 'LineWidth', 10), set(gca,'XScale','log'), title (StimSpacingNames{iSMD}, 'FontSize', 20), xlabel('Actual Voxel Characterisic Frequency (kHz)'), ylabel('Response Level'), ylim ([0 StimLev(i)]), xlim ([-20 20]);          
