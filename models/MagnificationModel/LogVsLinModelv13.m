...
for i = 1:length(compF)
    excPat  = funROEX(compF(i),compLev(i),C,@funF2NErb);
    figure(1)
...
function excPat = funROEX(compF,compLev,C,magDomain)
...
excPat.nErb = linspace(magDomain(LowFreq),magDomain(HighFreq),nVoxels); % Linearly space in magnification scale (ERB, FS, LOG, LIN)
...
excPat.excLev = 10*log10(max(excPat.excInt,1)); % Threshold intensity response and convert to dB
end
