...
for iVMD = 1:length(VoxelMagSpace)
    ...
    for iSMD = 1:length(StimMagDomain)
        ...
        for i = 1:length(StimFreq)
            ...
            area(data.VoxelProp(iVMD).VFreqC,SMD.VoxRes(i,:), 'FaceColor',[(StimFreq(i)./max(StimFreq)) 0.5 0.5]), set(gca,'XScale','log'), xlabel('pCF (kHz)'), ylabel('Response Level'), axis tight, ylim ([0 StimLev(i)]), xlim ([0 20]);
...
%                ylabel (StimSpacingNamesv2{iVMD}, 'FontSize', 15)
                text(250, 250, StimSpacingNames{iVMD}, 'FontSize', 15);
             end
        end
if iVMD == 1
        for i = 1:length(StimFreq)
