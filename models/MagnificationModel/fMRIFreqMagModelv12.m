...
for iVMD = 1:length(VoxelMagSpace)
    ...
    for iSMD = 1:length(StimMagDomain)
        ...
        set(gca,'YScale','log'), xlabel('Frequency Gradient (mm)'), ylabel('Characterisic Frequency (kHz)'),title ('Log'),legend('Actual','Measured','Location','SouthEast')
    end
end

% subplot(2,2,1);
% plot (data.VMD{i,1}{1,1}.VCharaFreq)
% subplot(2,2,2);
% plot (data.VMD{i,1}{2,1}.VCharaFreq)
% subplot(2,2,3);
% plot (data.VMD{i,1}{3,1}.VCharaFreq)
% subplot(2,2,4);
% plot (data.VMD{i,1}{4,1}.VCharaFreq)
end
% figure
% subplot(2,2,1);
