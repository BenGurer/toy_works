...
for iTonoMap = 1:length(TonotopicMagnification)
    ...
    for iStimSet = 1:length(StimulusSetSpacing)
        ...
        ESTCF = lcfCalculatepCFEstimation (StimulusSet(iStimSet,:),NeuronalResponse,StimulusSetSpacing{iStimSet},StimulusSetFreq{iStimSet});
        TonotopicMap(iTonoMap).ESTCF(iStimSet,:) = ESTCF';

        %         %% Calculate Root Mean Squared Error for all voxels
        SE=(log10(NeuronalResponse(iStimSet,:)) - log10(NeuronalProperties.CF)).^2;
        MSE = mean(SE);
...
if fESTCFSel == 1;
    ...
    for iTonoMap = 1:length(TonotopicMagnification)
        ...
        for iStimSet = 1:length(StimulusSetNames)
            ...
            scatter (TonotopicMap(iTonoMap).D,EstCF, 50, SpacingColour{iStimSet}, 'fill');
        end
        set(gca,'YScale','log');
