...
for iTonoMap = 1:length(TonotopicMagnification)
    ...
    NeuronalProperties = lcfCreateTonotopicProperties(Fn,TWn,f,TuningWidth{iTonoMap},TonotopicMagnification{iTonoMap},TonoMag2Freq{iTonoMap},C);
    ...
    % percentage of total point images
    %
    %% centre around 2 kHz
    %%
    PointImagePercentPerVoxel = 0.02;
    NeuronsPerVoxel = length(NeuronalProperties.CF) * PointImagePercentPerVoxel;
    nVoxels = length(NeuronalProperties.CF) / NeuronsPerVoxel;
    for z = 1:nVoxels
    ...
    for iStimSet = 1:length(StimulusSetSpacing)
        ...
        ESTCF = lcfCalculatepCFEstimation (StimulusSet(iStimSet,:),VoxelResponse,StimulusSetSpacing{iStimSet},StimulusSetFreq{iStimSet});
        TonotopicMap(iTonoMap).ESTCF(iStimSet,:) = ESTCF';
        
        %         %% Calculate Root Mean Squared Error for all voxels
        SE=(log10(VoxelResponse(iStimSet,:)) - log10(pCF)).^2;
        MSE = mean(SE);
...
if fESTCFSel == 1;
    ...
    for iTonoMap = 1:length(TonotopicMagnification)
        ...
        for iStimSet = 1:length(StimulusSetNames)
            EstCF = TonotopicMap(iTonoMap).ESTCF(iStimSet,:);
            %         EstpCF = [zeros(1,xmin), EstpCF(xmin+1:xmax), zeros(1,length(EstpCF) - xmax)];
            scatter (1:nVoxels,EstCF, 50, SpacingColour{iStimSet}, 'fill');
        end
        set(gca,'YScale','log');
        xlabel('Point Image', 'FontSize', fFontSize), ylabel('pCF (kHz)', 'FontSize', fFontSize);
        fLeg = legend('Actual',...
...
if fStimulusvsTonotopicSel == 1
    ...
    for iStimSet = 1:length(StimulusSetSpacing)
        for i = 1:length((StimulusSet(iStimSet,:)))
            figure (fStimTono)% plot to stimulus properties figure
            subplot(2,2,iStimSet); % create and index subplot
            plot(repmat(StimulusSet(iStimSet,i),1,2),[min(ylim) StimulusLevel(iStimSet,i)], 'color',[(StimulusSet(iStimSet,i))./max(StimulusSet(iStimSet,:)) 0.5 0.5], 'LineWidth', 10); % at each stimulus frequency create lines the level of each stimulus presenation level
            % set plot properties
            set(gca,'XScale','log');
...
%% Local Functions
function properties = lcfCreateTonotopicProperties(Fn,TWn,f,G,Gn,Gi,C)
...
x1 = A * (Gn(f0)) + B;
x2 = A * (Gn(f1)) + B;
x1 = round(x1,1,'significant');
x2 = round(x2,1,'significant');
step = (x2-x1)*StepPercentage;
