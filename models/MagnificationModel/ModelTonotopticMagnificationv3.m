...
StimMagDomain = {str2func('@LinMagDom'),str2func('@funNDLF'),str2func('@lcfNErb'),str2func('@LogMagDom')};
StimMagDomain2Freq = {str2func('@LinMagDom2Freq'),str2func('@funInvNDLF'),str2func('@lcfInvNErb'),str2func('@LogMagDom2Freq')};
...
% function y = FreqDisMagDom(x)
% %% number of Freq Selectivity units
% a = 0.0214; k = -0.15; m = 5.056; SL = 35;
% % f = f.*1000;
% y = exp(a .* sqrt(f) + k + m.*(SL.^-1));
% % y = exp(-0.676727 .* sqrt(x)).*(-4.39147-2.97183 .* sqrt(x));
% % y = 92.9414 .* exp((0.0214 .* sqrt(f))) .* (-46.729 + sqrt(f));
% 
% df = -((2.*(a.*sqrt(f) + 1) .* exp (-(a.* SL .* sqrt(f) + k .*SL + m)./SL))./a.^2
% 
% % log df = a*sqrt(f)+B) Weir 1977
% % x = f
% % integral of 1/e^(a*sqrt(x)+B)
% a = 0.026, b = -0.533; % for 40 dB SL
% y = -((2.*(a.*sqrt(x) +1).*exp(a.*(-sqrt(x)-b))./a.^2));
% end
% function f = FreqDisMagDom2Freq(x)
% a = 0.0214; k = -0.15; m = 5.056; SL = 35;
% 
% f = ((log(x) - (k + m.*(SL.^-1)))./a).^2;
% % y = 92.9414 .* log((0.0214 .* sqrt(x))) .* (-46.729 + sqrt(x));
% % f = f./1000;
% end
...
function f = lcfunInvNDLF (nDLF)
fs = 0:0.01:20;
% convert from kHz to Hz
fs = fs.*1000;
a = 0.026;
b = -0.533;
dcortex = 0:0.001:30; % distance of cortex
d = -(2.^(1-b-a .* sqrt(fs)) .* 5.^(-b-a .* sqrt(fs)) .* (1+a .* sqrt(fs) .* log(10)))./(a.^2 .* (log(10)).^2);
end
