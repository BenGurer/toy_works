...
for iTonoMap = 1:length(TonotopicMagnification)
    ...
    %% centre around 2 kHz
    %%
    % we want certain number of voxels per mm of cortex
    % want to set cortical distance and number per mm
...
%     TonotopicMap(iTonoMap).pCF = pCF;
    ...
    for iStimSet = 1:length(StimulusSetSpacing)
        ...
        SE=(log10(abs(ESTCF')) - log10(abs(pCF))).^2;
        MSE = mean(SE);
