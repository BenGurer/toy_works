...
TonotopicMap = struct('SpacingOrder',{'DLM','ERB','Log'},...
    'CF',zeros(length(TonotopicMagnification),nNeurons),....
'p',zeros(length(TonotopicMagnification),nNeurons),....
'Qnr',zeros(length(TonotopicMagnification),nNeurons),...
'ESTpCF', zeros(length(TonotopicMagnification))  );

% ,length(StimulusSetSpacing),nVoxels

    TonotopicMap(iTonoMap).RMSE = zeros(1,length(StimulusSetSpacing));
