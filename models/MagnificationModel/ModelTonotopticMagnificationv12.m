...
for iTonoMap = 1:length(TonotopicMagnification)
    ...
    for n = 1:nVoxels
        pCF(n) = mean (NeuronalProperties.CF(1 + (NeuronsPerVoxel .* (n-1)) : (NeuronsPerVoxel .* (n-1)) + NeuronsPerVoxel));
    end
    TonotopicMap(iTonoMap).pCF
    
    %% Present each Stimulus Set in turn to neuronal population
    for iStimSet = 1:length(StimulusSetSpacing)
        for i = 1:length(StimulusSet)
            NeuronalResponse(i,:) = lcfModelNeuronalResponse(StimulusSet(i),StimulusLevel,NeuronalProperties.CF,NeuronalProperties.p,NoiseLev);
        ...
        TonotopicMap(iTonoMap).(iStimSet).ESTpCF = ESTpCF;
    end
end
%% Display results

%% Local Functions
    function properties = lcfCreateTonotopicProperties(LowFreq,HighFreq,nNeurons,C,TonotopicMagnification,TonoMag2Freq,TuningWidth,nr);    %(LowFreq,HighFreq,nVoxels,C,magDomain,magScale2F,magFilterWidth)
                TonotopicMagnification = linspace(TonotopicMagnification(LowFreq),TonotopicMagnification(HighFreq),nNeurons); % Linearly space in magnification scale domain (ERB, FS, LOG, LIN)
        % Characterisic Frequencies
        properties.CF = TonoMag2Freq(TonotopicMagnification);   % neuron Characterisic Frequency - Convert maginification scale values to frequency values (kHz)
        % Tuning Width
        Q = TuningWidth(CF); % Neuron frequency tuning width for each Voxel Charactersic Frequency (for each neuron)(Q)
    ...
    function y = lcfLinearMagnification(f)
        y = f;
    end
    function f = lcfLinMag2Freq(x)
        f = x;
    end
    function y = lcfLogarithmicMagnification(f)
        y = log10(f);
    end
