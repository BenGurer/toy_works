...
fFontSize = 20;
graphselect = 1; % 1 is for entire frequency range - else zoomed
...
for iTonoMap = 1:iTonoMap
    ...
    pCF = pCF(xmin+1:xmax);
    data.nVoxelsWithinStimLim (iTonoMap) = xmax - xmin;
    for iStimSet = 1:length(StimulusSetNames)
        EstpCF = TonotopicMap(iTonoMap).ESTpCF(iStimSet,:);
...
for iTonoMap = 1:iTonoMap
    MSE = zeros(1,(iStimSet));
    xmin = find(data.VoxelProp(iTonoMap).VFreqC <= StimLowFreq,1);
    xmax = find(data.VoxelProp(iTonoMap).VFreqC >= StimHighFreq,1);  % highest frequency to test
    pCF = data.VoxelProp(iTonoMap).VFreqC;
    pCF = pCF(xmin+1:xmax);
    data.VoxelsWithinStimLim (iTonoMap) = xmax -xmin;
    for iStimSet = 1:iStimSet
        EstpCF = data.VMD{iTonoMap,1}{iStimSet ,1}.VCharaFreq;
        EstpCF = EstpCF(xmin+1:xmax);
        SE = zeros(iStimSet,length(pCF));
        for i = 1:length(pCF)
            SE(iStimSet,i)= (pCF(i) - EstpCF(i)).^2;
        ...
        MSE(iStimSet) = mean(SE(iStimSet,:));
        data.RMSEStimLim(iTonoMap,iStimSet) = sqrt(MSE(iStimSet));
    end
end
