...
TonotopicMagnification = {str2func('@lcfNDLF'),str2func('@lcfNDLFe') ,str2func('@lcfNErb')};
TonoMag2Freq = {str2func('@lcfInvNDLF'),str2func('@lcfInvNDLFe'),str2func('@lcfInvNErb')};
TuningWidth = { str2func('@lcfDLF'),str2func('@lcfDLFe'),str2func('@lcfErb')};
TonotopicNames = {'Frequency Discrimination','Frequency Discrimination - e','Equivalent Rectangular Bandwidth'};   % names for graph titles for each stimulus spacing - must be in same order as StimMagDomain and StimMagDomain2Freq variables
...
for iTonoMap = 1:length(TonotopicMagnification)
    ...
    if iTonoMap == 3
        % only normalise DLM p values (filter coeffients) - 2 is the place
    ...
    %     TonotopicMap(iTonoMap).pcompCF = NeuronalProperties.CF(NeuronalProperties.CF >0.9 & NeuronalProperties.CF <1.1);
    %     TonotopicMap(iTonoMap).pcomp = NeuronalProperties.p(NeuronalProperties.CF >0.9 & NeuronalProperties.CF <1.1);
    
    
    
    %     plot (ans(1).pcompCF,ans(1).pcomp)
    % figure
    % plot (ans(1).pcompCF,ans(1).pcomp)
    % hold on
    % plot (ans(2).pcompCF,ans(2).pcomp)
    
    
    %     TonotopicMap(iTonoMap).por = NeuronalProperties.por;
    %     TonotopicMap(iTonoMap).pdif = max(NeuronalProperties.p - NeuronalProperties.por);
    for z = 1:nVoxels
...
for iTonoMap = 1:length(TonotopicMagnification)
    ...
    fLeg = legend((TonotopicNames{1}),(TonotopicNames{2}),(TonotopicNames{3}),'Location','SouthEast');
    set(fLeg, 'FontSize', fFontSize./2);
end
fTuningvsCF = figure('Color', [1 1 1]);
for iTonoMap = 1:length(TonotopicMagnification)
    plot (TonotopicMap(iTonoMap).p,TonotopicMap(iTonoMap).CF, 'color',SpacingColour{iTonoMap}, 'LineWidth', 2)
    hold on
    set(gca,'YScale','log');
    xlabel('p', 'FontSize', fFontSize), ylabel('CF (kHz)', 'FontSize', fFontSize);
    warning('off','MATLAB:legend:IgnoringExtraEntries')
    fLeg = legend((TonotopicNames{1}),(TonotopicNames{2}),(TonotopicNames{3}),'Location','SouthEast');
    set(fLeg, 'FontSize', fFontSize./2);
end
...
Qnr = Q ./ nr;    % normalise Tuning width at 1 kHz by ERB
properties.p = C*4*properties.CF./Qnr; % Filter coefficient of each Voxel % VTuningWidth - population tuning width / populationERB
