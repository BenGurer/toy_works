...
StimFreqs = linspace(StimLowFreq,StimHighFreq,StimSteps);
% compF = 1;
compLev= ones(size(StimFreqs));
C=0.06;
plotC = {'k','b','r','g','y'}; % Cell array of colours.

plotC = repmat(plotC,1,round(length(StimFreqs))./(StimSteps./2));
Response = cell(size(StimFreqs));
for i = 1:length(StimFreqs)
    Response{i}  = funROEX(StimFreqs(i),compLev(i),C,@funF2NErb,@funNErb2F,@funErb);
...
fMRIdata = 
plot (StimFreqs,Response {:,1}.excLev(6))
