...
for iVMD = 1:length(VoxelMagSpace)
    ...
    for iSMD = 1:length(StimMagDomain)
        ...
        if iVMD == 1
            for i = 1:length(StimFreq)
                % plot to stimulus figure
                figure (fStim)
                subplot(length(VoxelMagSpace)./2,length(StimMagDomain)./2,iSMD);
                plot(repmat(StimFreq(i),1,2),[min(ylim) StimLev(i)], 'color',[(StimFreq(i)./max(StimFreq)) 0.5 0.5], 'LineWidth', 10);
                set(gca,'XScale','log');
                title (StimSpacingNames{iSMD}, 'FontSize', 20), xlabel('Frequency (kHz)'), ylabel('Response Level');
