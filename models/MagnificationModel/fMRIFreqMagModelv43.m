    ...
    for iSMD = 1:length(StimMagDomain)
        ...
        for i = 1:length(StimFreq)
            ...
            if iSMD == 3 && i == 5 && iVMD == 3
                ...
                %     set(gca,'YScale','log');
                xlabel('Tonotopic Gradient (mm)', 'FontSize', fFontSize), ylabel('Population Characteristic Frequency pCF (kHz)', 'FontSize', fFontSize);
...
for iVMD = 1:length(VoxelMagSpace)
    ...
    for iSMD = 1:length(StimMagDomain)
        ...
        set(gca,'YScale','log');
        xlabel('Tonotopic Gradient (mm)', 'FontSize', fFontSize), ylabel('pCF (kHz)', 'FontSize', fFontSize);
        warning('off','MATLAB:legend:IgnoringExtraEntries')
