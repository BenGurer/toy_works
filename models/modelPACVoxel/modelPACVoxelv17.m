...
%                 end
            ...
            SE=(log10(estCtrd(I,II)) - log10(Ctrd)).^2;
            MSE = mean(SE);
            Ctrdrmse(I,II)= sqrt(MSE);
            
            estSprd(I,II) = mean(eSprd(I,II,:));
            
            SE=(log10(estSprd(I,II)) - log10(Sprd)).^2;
            MSE = mean(SE);
...
%             efwhm(I,II) = 2 * sqrt(2*log(2)) * fitC(I,II);
            ...
            text(max(xlim),max(ylim),sprintf('Ctrd = %.2f (%.2f) kHz\nSprd = %.2f (%.2f) kHz\nCF = %.2f (%.2f) kHz\nTW = %.2f (%.2f) kHz\nx0 CF=%.2f TW=%.2f\nRMSE Ctrd = %.4f Sprd = %.4f\nRMSE CF = %.4f TW = %.4f',Ctrd(II),estCtrd(I,II),Sprd(II),estSprd(I,II),CF(II),fitCF(I,II),TW(II),fitTW(I,II),max_stimF(I,II),fwhm(I,II),Ctrdrmse(I,II),Sprdrmse(I,II),CFrmse(I,II),fitTWrmse(I,II)),'HorizontalALignment','right','VerticalAlignment','top')
