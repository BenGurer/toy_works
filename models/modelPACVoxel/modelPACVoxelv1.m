function [dCtrd,dSprd,eCtrd,eSprd,fitCF,fitC,fiterror] = modelPACVoxelv1 (NOI)
close all; clf;
...
fitCF = zeros(NY,NX); fitC = zeros(NY,NX); fiterror = zeros(NY,NX);
for I = 1:NY
...
ub= [20, 5];
fun = @lcfROEX;
options = optimoptions('lsqcurvefit','TolFun',1e-8); % default=1e-6
...
figure(fFit)
        subplot(NY,NX,(NY-I)*NX+II), hold on
        plot(fs,fitW,'k-'), plot(stimF,r,'ro--')
    end
end
