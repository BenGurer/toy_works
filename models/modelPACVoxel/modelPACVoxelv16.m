...
%         end
        ...
        for II = 1:npTW
            ...
            SE=(log10(Ctrd(I,II)) - log10(Ctrd)).^2;
            MSE = mean(SE);
            ...
            text(max(xlim),max(ylim),sprintf('Ctrd = %.2f (%.2f) kHz\nSprd = %.2f (%.2f) kHz\nCF = %.2f (%.2f) kHz\nTW = %.2f (%.2f) kHz\nx0CF = %.2f\nx0TW = %.2f\nCtrdRMSE = %.4f\nSprdRMSE = %.4f\nCFRMSE = %.4f\nTWRMSE = %.4f',Ctrd(II),estCtrd(I,II),Sprd(II),estSprd(I,II),CF,fitCF(I,II),TW(II),fitTW(I,II),max_stimF(I,II),fwhm(I,II),Ctrdrmse(I,II),Sprdrmse(I,II),CFrmse(I,II),fitTWrmse(I,II),'HorizontalALignment','right','VerticalAlignment','top'))
            set(gca,'XScale','log');
