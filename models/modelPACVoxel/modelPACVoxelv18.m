...
C = [percent/4 percent percent*10];
C = C/100;
TW = CF * C;
...
%         end
        ...
        for II = 1:npTW
            for j = 1:nErrorRepeats
                ...
                % from actual
                eCtrd(I,II,j) = nanmean(jwd1); eSprd(I,II,j) = nanmean(jwd2);
...
%                 end
                ...
                elseif numel(fwhmIr)== 0
                    ...
                    fwhm(I,II) = max(stimF(fwhmIr)) - min(stimF(fwhmIr));
                ...
                ofitCF(I,II,j) = x(1);
                ofitC(I,II,j) = x(2);
                ofitTW(I,II,j) = ofitCF(I,II,j) .*ofitC(I,II,j);
                ofiterror(I,II)= error;
            end
            
            
            estCtrd(I,II) = mean(eCtrd(I,II,:));
            SE=(log10(estCtrd(I,II)) - log10(Ctrd(II))).^2;
            MSE = mean(SE);
            Ctrdrmse(I,II)= sqrt(MSE);
            
            estSprd(I,II) = mean(eSprd(I,II,:));
            
            SE=(log10(estSprd(I,II)) - log10(Sprd(II))).^2;
            MSE = mean(SE);
            Sprdrmse(I,II)= sqrt(MSE);
            
            
            fitCF(I,II) = mean(ofitCF(I,II,:));
            
            SE=(log10(fitCF(I,II)) - log10(CF)).^2;
            MSE = mean(SE);
            CFrmse(I,II)= sqrt(MSE);
            
            fitC(I,II) = mean(ofitC(I,II,:));
            fitTW(I,II) = fitCF(I,II) .*fitC(I,II);
            
            SE=(log10(fitTW(I,II)) - log10(TW)).^2;
            MSE = mean(SE);
            fitTWrmse(I,II)= sqrt(MSE);
...
%             text(max(xlim),max(ylim),sprintf('Ctrd = %.2f (%.2f) kHz\nSprd = %.2f (%.2f) kHz\nCF = %.2f (%.2f) kHz\nTW = %.2f (%.2f) kHz\nx0 CF=%.2f TW=%.2f\nRMSE Ctrd = %.4f Sprd = %.4f\nRMSE CF = %.4f TW = %.4f',Ctrd(II),estCtrd(I,II),Sprd(II),estSprd(I,II),CF,fitCF(I,II),TW(II),fitTW(I,II),max_stimF(I,II),fwhm(I,II),Ctrdrmse(I,II),Sprdrmse(I,II),CFrmse(I,II),fitTWrmse(I,II)),'HorizontalALignment','right','VerticalAlignment','top')
%             text(max(xlim),max(ylim),sprintf('Ctrd = %.2f (%.2f) kHz\nSprd = %.2f (%.2f) kHz',Ctrd(II),estCtrd(I,II),Sprd(II),estSprd(I,II)))
%             text(max(xlim),max(ylim),sprintf('Ctrd = %.2f (%.2f) kHz\nSprd = %.2f (%.2f) kHz\nCF = %.2f (%.2f) kHz\nTW = %.2f (%.2f) kHz\nx0 CF=%.2f TW=%.2f\nRMSE\nCtrd = %.4f Sprd = %.4f\nCF = %.4f TW = %.4f',Ctrd(II),estCtrd(I,II),Sprd(II),estSprd(I,II),CF,fitCF(I,II),TW(II),fitTW(I,II),max_stimF(I,II),fwhm(I,II),Ctrdrmse(I,II),Sprdrmse(I,II),CFrmse(I,II),fitTWrmse(I,II)))
...
T = table(CF,TW,percent,Sprd,estCtrd,Ctrdrmse,estSprd,Sprdrmse,fitCF,CFrmse,fitTW,fitTWrmse,...
    'VariableNames',{'CharacteristicFrequency' , 'TuningWidth', 'TWPercent','Spread', 'EstCentriod', 'CentriodRMSE', 'EstSpread','SpreadRMSE', 'FittedCF','CFRMSE','FittedTW','TWRMSE'})
