function [ x, tw_Deconv ] = cal_centredTWROIAverage(e,t,analysisParams)
    ...
    %      by: Ben Gurer
    %    date: 05/15/2018
    % purpose: calculate average TW within given ROI
    %   input: voxel estimates from ROI, analysis parameters
    %  output: centred TW estimate and fitted gaussian params
    %
%% 
...
tw_Deconv = HRF_TW_est(:,hdrMaxTimePoint)/max(HRF_TW_est(:,hdrMaxTimePoint));
freqBin = [1-nOverlays : 1/resolution : nOverlays-1]';

p = [0 0.5]; %guess
opts = optimset('MaxFunEvals', 500, 'Display', 'off');
[x_g, resnorm, ~, exitflag, output] = lsqcurvefit(@get_Gaussian, p, freqBin, tw_Deconv, [], [], opts);
[x_r, resnorm, ~, exitflag, output] = lsqcurvefit(@get_ROEX, p + 8, freqBin + 8, tw_Deconv, [], [], opts);

figure
plot(freqBin,tw_Deconv)
hold on
plot(freqBin,get_Gaussian(x_g,freqBin))
plot(freqBin,get_ROEX(x_r,freqBin + 8))
legend('deconv','Gaussian','ROEX')
