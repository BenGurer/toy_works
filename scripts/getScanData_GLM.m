...
for iScan = 1:length(analysisNames_Scans)
    ...
    eval(['data.scan_GLMdata{iScan} = getGroupAnalysisData(thisView,analysisNames_Scans);'])
end
% end
...
for iGroup = 1:length(groupNames)
    ...
    for iAnal = 1:length(analysisNames_Groups)
        eval(['data.' groupNames{iGroup} '.group_GLMdata{iAnal} = getGroupAnalysisData(thisView,analysisNames_Groups);'])
    end
    %
