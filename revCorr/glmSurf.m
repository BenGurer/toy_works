...
% figure;
% names = {'Voxel', 'AC', 'ACHighR2'};
% xName = 'Time (s)';
% yName = 'Condition';
% zName = 'Signal Change %';
% for i = 1:length(estHDR)
%     ehdr = estHDR{i};
%     pehdr=permute(ehdr,[2,1]);
%     [nStim, ncol] = size(pehdr);
%     mTime=max(e.time);
%     lzLim = min(min(pehdr));
%     uzLim = max(max(pehdr));
%     nPlots = 3;
%     
%     subplot(length(estHDR),nPlots,i)
%     imagesc(e.time,1:nStim,pehdr);
%     title(names{i})
%     axis tight; xlabel(xName); ylabel(yName);
% %     axis([1 mTime 1 nStim])
%     
%     subplot(length(estHDR),nPlots,i+nPlots)
%     waterfall(e.time,1:nStim,pehdr);
%     axis tight; xlabel(xName); ylabel(yName); zlabel(zName);
% %     axis([1 mTime 1 nStim lzLim uzLim])
%     
%     subplot(length(estHDR),nPlots,i+nPlots*2)
%     surf(e.time,1:nStim,pehdr);
%     axis tight; xlabel(xName); ylabel(yName); zlabel(zName);
% %     axis([1 mTime 1 nStim lzLim uzLim])
% end
...
% 
% figure;
% names = {'Voxel', 'RightAC', 'RightACHighR2'};
% for i = 1:length(estHDR)
%     ehdr = estHDR{i};
%     pehdr=permute(ehdr,[2,1]);
%     [nStim, ncol] = size(pehdr);
...
%     subplot(length(estHDR),nPlots,i+nPlots)
%     waterfall(pehdr);
%     axis([1 ncol 1 nStim lzLim uzLim])
%     
%     subplot(length(estHDR),nPlots,i+nPlots*2)
%     surf(pehdr);
%     axis([1 ncol 1 nStim lzLim uzLim])
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function saveEhdr(e,roi,i)
if ieNotDefined('roi')
else
    filename = roi{i}.name;
save(['eHDR_' filename],'e','roi');
end
