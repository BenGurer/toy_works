...
%
%        $Id:$ 
%      usage: pRFGUI_auditory()
%         by: justin gardner / adapted for auditorysensory
%       date: 11/20/11
%    purpose: GUI for getting params for pRF
%
%%
function params = pRFGUI_auditory(varargin)
...
if ~pRFFitParamsOnly
  ...
  paramsInfo{end+1} = {'saveName','pRF_auditory','File name to try to save as'};
...
paramsInfo{end+1} = {'rfType',{'gaussian','ROEX'},'Type of pRF fit. Gaussian fits a gaussian with x,y,width as parameters to each voxel. gaussian-hdr fits also the hemodynamic response with the parameters of the hdr as below.'};
...
% paramsInfo{end+1} = {'designSupersampling',4,'minmax=[0 100]','incdec=[-1 1]','Supersample design matrix'};
% paramsInfo{end+1} = {'acquisitionDelay',0.75,'minmax=[0 100]','incdec=[-1 1]','Image acquistion delay'};
